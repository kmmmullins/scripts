...
if [ -z "${PLATFORM_NAME:-""}" -o -z "${PLATFORM_RELEASE:-""}" ]; then
    ...
    # Test for Solaris.
        ...
        if grep -qi 'amazon linux' /etc/system-release; then
            ...
            fail "$(cat /etc/system-release) is not a supported platform for Puppet Enterprise v3.3.2
        ...
        fail "$(uname -s) is not a supported platform for Puppet Enterprise v3.3.2
            Please visit http://links.puppetlabs.com/puppet_enterprise_${PE_LINK_VER?}_platform_support to request support for this platform."
    fi
fi
...
url="https://waldorf.mit.edu:8140/packages/3.3.2/${PLATFORM_TAG}.bash"
...
The required agent packages should be retrieved when puppet runs on the master, after which you can run the install.bash script again."
        ...
        fail "This method of agent installation is not supported for ${PLATFORM_TAG?} in Puppet Enterprise v3.3.2
Please install using the puppet-enterprise-installer from the Puppet Enterprise v3.3.2 tarball"
    fi
fi
