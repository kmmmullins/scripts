...
ssh oastest@$node '/oracle/product/10gR2/opmn/bin/opmnctl status | grep Instance'
if [ $? -gt 0 ]; then
    echo " "
...
ssh oastest@$node '/oracle/product/forms/opmn/bin/opmnctl status | grep Instance'
if [ $? -gt 0 ]; then
    echo " "
