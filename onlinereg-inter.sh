...
for i in `seq 1 1000`;
    ...
    echo $node;
    echo "  "    
    ssh root@$node "grep agreement /var/log/httpd/registration-secure_access_log.20120203 | grep POST | wc" 
    ssh root@$node "grep submit_registration /var/log/httpd/registration-secure_access_log.20120203 | grep POST | wc" 
